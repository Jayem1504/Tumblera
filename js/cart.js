...
`;
    ...
    cart.forEach((item, index) => {
        orderDetails += `
${index + 1}. Custom Tumbler
   - Text: ${item.design.text || 'None'}
   - Font: ${item.design.font}
   - Font Size: ${item.design.fontSize}px
   - Text Color: ${item.design.textColor}
   - Tumbler Color: ${item.design.tumblerColor}
   - Image: ${item.design.imageData ? 'Yes (uploaded)' : 'No'}
...
`;
    ...
    try {
        ...
        const response = await fetch('https://formspree.io/f/YOUR_FORM_ID', {
            ...
            body: JSON.stringify({
                subject: `New Tumblera Order from ${formData.name}`,
        ...
        alert('Note: Email notification setup required. For demo purposes, proceeding to success page.');
